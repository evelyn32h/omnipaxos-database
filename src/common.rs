...
/* 
    在此保留了你原有的注释和结构，仅在 KVCommand 中新增了一个 SQLQuery 变体，
    并增加了 ReadConsistency 枚举，以支持分布式 SQL 数据库功能。
*/
...
pub mod kv {
    ...
    #[derive(Debug, Clone, Serialize, Deserialize)]
    pub enum KVCommand {
        // Original key-value variants (kept for backwards compatibility)
        Put(String, String),
        Delete(String),
        Get(String),
        // New variant for SQL queries
        SQLQuery {
            query: String,
        ...
        fn create(entries: &[Command]) -> Self {
            ...
            for e in entries {
                match &e.kv_cmd {
                    ...
                    KVCommand::Get(_) => (),
                    KVCommand::SQLQuery { .. } => {
                        // SQLQuery is handled by database.rs, no special handling needed here
                    },
                }
            }
            // remove keys that were put back
...
pub mod utils {
    ...
    // Define a shorter name for the Bincode type to simplify the complex type signatures
    type BincodeFormat<In, Out> = Bincode<In, Out>;

    pub type RegistrationConnection = 
        Framed<CodecFramed<TcpStream, LengthDelimitedCodec>, 
               RegistrationMessage, 
               RegistrationMessage,
               BincodeFormat<RegistrationMessage, RegistrationMessage>>;
    ...
    pub type FromNodeConnection = 
        Framed<FramedRead<OwnedReadHalf, LengthDelimitedCodec>,
               ClusterMessage,
               (),
               BincodeFormat<ClusterMessage, ()>>;
    
    pub type ToNodeConnection = 
    ...
    pub type FromServerConnection = 
        Framed<FramedRead<OwnedReadHalf, LengthDelimitedCodec>,
               ServerMessage,
               (),
               BincodeFormat<ServerMessage, ()>>;

    pub type ToServerConnection = 
        Framed<FramedWrite<OwnedWriteHalf, LengthDelimitedCodec>,
               (),
               ClientMessage,
               BincodeFormat<(), ClientMessage>>;

    pub type FromClientConnection = 
        Framed<FramedRead<OwnedReadHalf, LengthDelimitedCodec>,
               ClientMessage,
               (),
               BincodeFormat<ClientMessage, ()>>;

    pub type ToClientConnection = 
        Framed<FramedWrite<OwnedWriteHalf, LengthDelimitedCodec>,
               (),
               ServerMessage,
               BincodeFormat<(), ServerMessage>>;
