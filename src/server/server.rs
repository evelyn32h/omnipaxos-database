...
impl OmniPaxosServer {
    pub async fn new(config: OmniPaxosKVConfig) -> Self {
        ...
        OmniPaxosServer {
            ...
            // Database::new() is now an async function that needs to be awaited
    ...
    async fn update_database_and_respond(&mut self, commands: Vec<Command>) {
        for command in commands {
            let maybe_response = self.database.handle_command(command.kv_cmd).await;
            if command.coordinator_id == self.id {
                let response = match maybe_response {
                    Some(result_string) => ServerMessage::Read(command.id, Some(result_string)),
    ...
    fn save_output(&mut self) -> Result<(), std::io::Error> {
        ...
        if let Some(parent) = std::path::Path::new(&self.config.local.output_filepath).parent() {
