...
impl Client {
    ...
    async fn send_request(&mut self, is_write: bool) {
        // Construct a key (e.g., "key_0", "key_1", ...)
        let key = format!("key_{}", self.next_request_id);
        
        // Based on whether it's a write operation, construct SQL query:
        // - For writes, use INSERT ... ON CONFLICT ... DO UPDATE
        // - For reads, use SELECT statement
        let query = if is_write {
            ...
            format!("SELECT value FROM kv WHERE key = '{}'", key)
        };

        // For read queries, choose consistency, e.g., reading from Leader; write operations don't need consistency
        let consistency = if is_write {
            None
        } else {
            Some(ReadConsistency::Leader)
        };

        let command = KVCommand::SQLQuery { query, consistency };
        let request = ClientMessage::Append(self.next_request_id, command);
